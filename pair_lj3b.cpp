...
PairLJ3B::PairLJ3B(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 0;
  restartinfo = 0;
  one_coeff = 0;
...
void PairLJ3B::compute(int eflag, int vflag)
{
...
void PairLJ3B::settings(int narg, char **/*arg*/)
{
...
void PairLJ3B::coeff(int narg, char **arg)
{
...
void PairLJ3B::init_style()
{
  if (atom->tag_enable == 0)
    error->all(FLERR,"Pair style LJ3B requires atom IDs");
  if (force->newton_pair == 0)
    error->all(FLERR,"Pair style LJ3B requires newton pair on");
...
double PairLJ3B::init_one(int i, int j)
{
...
void PairLJ3B::read_file(char *file)
{
  memory->sfree(params);
  params = nullptr;
  nparams = maxparam = 0;
  ...
  if (comm->me == 0) {
    PotentialFileReader reader(lmp, file, "lj3b", unit_convert_flag);
    char *line;
	...
	params[nparams].flagoffset  = values.next_int();
      ...
      if (params[nparams].epsilon < 0.0 || params[nparams].sigma < 0.0 ||
          params[nparams].cutoff < 0.0 || params[nparams].lambda < 0.0 ||
          params[nparams].powern < 0.0 ||
          !(params[nparams].flagoffset == 0 ||
          params[nparams].flagoffset == 1 ) )
        error->one(FLERR,"Illegal LJ3B parameter");

      if (params[nparams].flagoffset == 1 && params[nparams].lambda > 0.0 )
        error->one(FLERR,"LJ3B offset and lambda incompatible, both can't be ON");
      if (params[nparams].flagoffset == 1 && params[nparams].epsilon == 0.0 )
        error->one(FLERR,"LJ3B offset and epsilon incompatible, epsilon can't be 0");
      if (params[nparams].flagoffset == 1 && params[nparams].sigma == 0.0 )
        error->one(FLERR,"LJ3B offset and sigma incompatible, sigma can't be 0");
...
void PairLJ3B::setup_params()
{
  ...
  for (m = 0; m < nparams; m++) {
    params[m].cut = params[m].sigma*params[m].cutoff;

    params[m].c1 = pow(params[m].sigma,2*params[m].powern);
    params[m].c2 = pow(params[m].sigma,params[m].powern);
    params[m].c3 = 2*params[m].powern*pow(params[m].sigma,2*params[m].powern);
    params[m].c4 = params[m].powern*pow(params[m].sigma,params[m].powern);
    ...
    if (params[m].flagoffset == 1) {
                rtmp = params[m].sigma*pow(2,1.0/params[m].powern);
    }
...
void PairLJ3B::twobody(Param *param, double rsq, double &fforce,
                     int eflag, double &eng)
{
  double r,r2n,rn;

  r = sqrt(rsq);
  r2n = pow(r,-2*param->powern);
  rn = pow(r,-param->powern);
  fforce = 4.0 * param->epsilon * (param->c3*r2n - param->c4*rn);
  if (eflag) eng = 4.0 * param->epsilon * (param->c1*r2n - param->c2*rn) +
                   param->epsilon * param->flagoffset; // offset/shift energy modification
}

/* ---------------------------------------------------------------------- */

void PairLJ3B::threebody(Param *paramij, Param *paramik, Param *paramijk,
                       ...
                       double *fj, double *fk, int eflag, double &eng)
{
  double r1;
  double r2;
  double r2n1,rn1,fforce1,eng1;
  double r2n2,rn2,fforce2,eng2;

  r1 = sqrt(rsq1);
  r2n1 = pow(r1,-2*paramij->powern);
  rn1 = pow(r1,-paramij->powern);
  // twobody without epsilon (normalized) 
  fforce1 = 4.0 * (paramij->c3*r2n1 - paramij->c4*rn1);
  // twobody without epsilon (normalized)
  eng1 = 4.0 * (paramij->c1*r2n1 - paramij->c2*rn1);
//////////////////////////////////////////////////////////////////////////HEEERRREEE

  r2 = sqrt(rsq2);
  r2n2 = pow(r2,-2*paramik->powern);
  rn2 = pow(r2,-paramik->powern);
  // twobody without epsilon (normalized)
  fforce2 = 4.0 * (paramik->c3*r2n2 - paramik->c4*rn2);
  // twobody without epsilon (normalized)
  eng2 = 4.0 * (paramik->c1*r2n2 - paramik->c2*rn2);
